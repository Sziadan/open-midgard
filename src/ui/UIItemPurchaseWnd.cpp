...
UIItemPurchaseWnd::UIItemPurchaseWnd()
    : m_viewOffset(0),
      m_hoverRow(-1),
...
UIItemPurchaseWnd::~UIItemPurchaseWnd() = default;

void UIItemPurchaseWnd::SetShow(int show)
{
    ...
    int savedX = m_x;
    int savedY = m_y;
    if (LoadUiWindowPlacement("ItemPurchaseWnd", &savedX, &savedY)) {
        g_windowMgr.ClampWindowToClient(&savedX, &savedY, m_w, m_h);
        Move(savedX, savedY);
    }
}

bool UIItemPurchaseWnd::IsUpdateNeed()
{
    if (m_show == 0) {
        return false;
    }
...
void UIItemPurchaseWnd::StoreInfo()
{
    SaveUiWindowPlacement("ItemPurchaseWnd", m_x, m_y);
}

RECT UIItemPurchaseWnd::GetListRect() const
{
    return shopui::MakeRect(m_x + kListSideMargin, m_y + kListTop, m_w - kListSideMargin * 2, m_h - kListTop - kListBottomMargin);
}

int UIItemPurchaseWnd::GetVisibleRowCount() const
{
    return (std::max)(1, (m_h - kListTop - kListBottomMargin) / kRowHeight);
}

int UIItemPurchaseWnd::GetMaxViewOffset() const
{
...
int UIItemPurchaseWnd::HitTestDealRow(int x, int y) const
{
...
RECT UIItemPurchaseWnd::GetButtonRect(ButtonId buttonId) const
{
    switch (buttonId) {
    case ButtonAdd:
...
UIItemPurchaseWnd::ButtonId UIItemPurchaseWnd::HitTestButton(int x, int y) const
{
...
void UIItemPurchaseWnd::ActivateButton(ButtonId buttonId)
{
    switch (buttonId) {
    case ButtonAdd:
    ...
    case ButtonRemove:
        ...
        break;
    case ButtonConfirm:
        g_modeMgr.SendMsg(CGameMode::GameMsg_RequestShopBuyList, 0, 0, 0);
        break;
    case ButtonCancel:
        g_session.ClearNpcShopState();
        ClosePurchaseWindows();
        return;
    default:
        break;
    }
...
void UIItemPurchaseWnd::OnDraw()
{
    ...
    const RECT listRect = GetListRect();
    shopui::DrawFrameWindow(hdc, bounds, "Purchase");
    ...
    shopui::DrawWindowTextRect(hdc, shopui::MakeRect(headerRect.right - 64, headerRect.top + 1, 56, headerRect.bottom - headerRect.top - 2), "Cost", RGB(30, 30, 30), DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
    ...
    DrawButton(hdc, GetButtonRect(ButtonConfirm), "Buy", m_hoverButton == ButtonConfirm, m_pressedButton == ButtonConfirm);
...
void UIItemPurchaseWnd::OnLBtnDblClk(int x, int y)
{
    const int rowIndex = HitTestDealRow(x, y);
    if (rowIndex >= 0) {
        g_session.m_shopSelectedDealRow = rowIndex;
...
void UIItemPurchaseWnd::OnLBtnUp(int x, int y)
{
...
void UIItemPurchaseWnd::OnMouseMove(int x, int y)
{
...
void UIItemPurchaseWnd::OnWheel(int delta)
{
    const int step = delta > 0 ? -1 : 1;
    m_viewOffset = (std::max)(0, (std::min)(GetMaxViewOffset(), m_viewOffset + step));
}

void UIItemPurchaseWnd::HandleKeyDown(int virtualKey)
{
...
int UIItemPurchaseWnd::GetViewOffset() const
{
    return m_viewOffset;
}

int UIItemPurchaseWnd::GetHoverRow() const
{
    return m_hoverRow;
}

int UIItemPurchaseWnd::GetVisibleRowCountForQt() const
{
    return GetVisibleRowCount();
}

int UIItemPurchaseWnd::GetHoverButton() const
{
    return static_cast<int>(m_hoverButton);
}

int UIItemPurchaseWnd::GetPressedButton() const
{
    return static_cast<int>(m_pressedButton);
}

bool UIItemPurchaseWnd::GetButtonRectForQt(int buttonId, RECT* outRect) const
{
...
unsigned long long UIItemPurchaseWnd::BuildDisplayStateToken() const
{
